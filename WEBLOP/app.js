...
const elements = {
    ...
    bulkLoopCount: document.getElementById('bulkLoopCount'),
    bulkModeReverse: document.getElementById('bulkModeReverse'),
    bulkModeForward: document.getElementById('bulkModeForward'),
    bulkFcOff: document.getElementById('bulkFcOff'),
    bulkFcOn: document.getElementById('bulkFcOn'),
...
async function handleBatchVideoFiles(videoFiles) {
    ...
    // ログからの復元
    if (window._restoreFromLog) {
        applyBatchLogToVideos();
        window._restoreFromLog = false;
    }
...
function applyBulkSettings() {
    const count = parseInt(elements.bulkLoopCount.value) || 3;
    const mode = elements.bulkModeReverse.classList.contains('active') ? 'reverse' : 'forward';
    const fc = elements.bulkFcOn.classList.contains('active');

    batchLoopVideos.forEach(v => {
        v.loopCount = count;
        v.loopMode = mode;
        v.frameCut = fc;
    });
...
// ===== バッチ設定ログ (localStorage) =====
const BATCH_LOG_KEY = 'weblop_batch_log';

function saveBatchLog() {
    if (batchLoopVideos.length === 0) return;
    const log = {
        timestamp: Date.now(),
        videos: batchLoopVideos.map(v => ({
            name: v.name,
    ...
    try {
        localStorage.setItem(BATCH_LOG_KEY, JSON.stringify(log));
    } catch (e) { /* quota exceeded etc. */ }
}

function loadBatchLog() {
    try {
        const data = localStorage.getItem(BATCH_LOG_KEY);
        return data ? JSON.parse(data) : null;
    } catch (e) { return null; }
}

function applyBatchLogToVideos() {
    const log = loadBatchLog();
    if (!log || !log.videos) return;
    batchLoopVideos.forEach(v => {
        const match = log.videos.find(lv => lv.name === v.name);
        if (match) {
            v.loopCount = match.loopCount;
            v.loopMode = match.loopMode;
            v.frameCut = match.frameCut;
        }
    });
}
...
function showLogPanel() {
    ...
    if (!log || !log.videos || log.videos.length === 0) {
        content.innerHTML = '<div class="log-empty">保存された履歴がありません</div>';
    ...
    const modeLabel = (m) => m === 'reverse' ? 'Rev' : 'Fwd';
    const fcLabel = (fc, m) => m === 'reverse' ? (fc ? ' FC' : '') : '';

    content.innerHTML = `
        <div class="log-timestamp">保存: ${timeStr}</div>
        <div class="log-video-list">
            ${log.videos.map((v, i) => `
                <div class="log-video-row">
                    <span class="video-number">${i + 1}</span>
                    <span class="video-name">${v.name}</span>
                    <span class="video-setting">×${v.loopCount} ${modeLabel(v.loopMode)}${fcLabel(v.frameCut, v.loopMode)}</span>
                </div>
            `).join('')}
        </div>
        <button class="log-restore-btn" onclick="restoreFromLog()">同じファイルを選択して復元</button>
    `;
    panel.style.display = 'block';
}
...
function restoreFromLog() {
    hideLogPanel();
    // ファイル選択ダイアログを開く（復元フラグを立てる）
    window._restoreFromLog = true;
...
function setupEventListeners() {
    ...
    // 一括設定（ホバーで開閉）
    const bulkWrap = elements.bulkToggleBtn.parentElement;
    let bulkCloseTimer = null;
    const openBulk = () => {
        clearTimeout(bulkCloseTimer);
        elements.bulkDropdown.classList.add('open');
    };
    const closeBulk = () => {
        bulkCloseTimer = setTimeout(() => {
            elements.bulkDropdown.classList.remove('open');
        }, 200);
    };
    bulkWrap.addEventListener('mouseenter', openBulk);
    bulkWrap.addEventListener('mouseleave', closeBulk);
    // タッチ対応: タップでもトグル
    elements.bulkToggleBtn.addEventListener('click', () => {
        elements.bulkDropdown.classList.toggle('open');
    });

    elements.bulkModeReverse.addEventListener('click', () => {
        elements.bulkModeReverse.classList.add('active');
        elements.bulkModeForward.classList.remove('active');
    });
    elements.bulkModeForward.addEventListener('click', () => {
        elements.bulkModeForward.classList.add('active');
        elements.bulkModeReverse.classList.remove('active');
    });
    elements.bulkFcOff.addEventListener('click', () => {
        elements.bulkFcOff.classList.add('active');
        elements.bulkFcOn.classList.remove('active');
    });
    elements.bulkFcOn.addEventListener('click', () => {
        elements.bulkFcOn.classList.add('active');
        elements.bulkFcOff.classList.remove('active');
    });
    elements.bulkApplyBtn.addEventListener('click', () => {
        applyBulkSettings();
        elements.bulkDropdown.classList.remove('open');
    });
    ...
    elements.logCloseBtn.addEventListener('click', hideLogPanel);
}
